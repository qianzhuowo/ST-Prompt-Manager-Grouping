# Prompt Manager Grouping — 预设条目分组/收藏
...
### 📁 智能分组
- 根据提示词名称中的前缀（`【】` / `-` / 自定义包裹符 / 自定义分隔符）自动归类到不同分组
- 支持**两级分组**，适合管理大量提示词
- 可选择隐藏前缀，让列表更简洁
...
- **预设收藏**: 可以收藏置顶常用预设，便于快速切换，PMG提供快速切换面板

## 📖 使用方法

### 如何给提示词分组？

只需按照特定格式**命名**你的提示词条目，插件会自动识别并分组。支持两种命名方式：
...
在设置面板中启用「自定义包裹前缀」，并设置左右包裹符（例如：`「` 与 `」`），即可使用类似 `【】` 的包裹式前缀。
...
在设置面板中启用「自定义分隔符前缀」（例如设置为 `=`），即可使用类似 `-` 的分割式前缀。
...
安装后，在 SillyTavern 预设面板中找到 **「PMG设置」**，可以自由开关以下功能：
...
| 禁用原生拖拽排序 | 是否在启用分组时禁用 SillyTavern 原生拖拽排序 |
| 启用收藏 | 是否在条目右侧显示 ⭐ 按钮 |
| 内联收藏栏 | 是否在预设面板内显示收藏面板 |
| 浮动收藏快捷栏 | 是否显示页面右下角的浮动收藏按钮 |
| 收藏预设 | 是否启用预设收藏和快速切换功能 |

- **快速编辑前缀**：批量修改当前预设条目的名称前缀（支持多选）；该操作会直接写入预设数据，请谨慎使用
...
- 分组/隐藏前缀仅影响显示效果，**不会修改预设内容和数据**；但「快速编辑前缀」会修改预设条目名称
